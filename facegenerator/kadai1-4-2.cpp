...
int main(int argc, char *argv[])
{
    ...
    for( int i = 0; i < INIT_TIME; i++){
        ...
        cv::accumulate(tmp, avg_img);// 画像全体を累算器(avg_img)に加える
    ...
    for( int i = 0; i < INIT_TIME; i++){
        ...
        cv::pow(tmp_img, 2.0, tmp_img);//各要素の累乗を計算する
        tmp_img.convertTo(tmp_img, -1,2.0);//出力行列に要求するビット深度 -1なら元の画像と同じ
        cv::sqrt(tmp_img, tmp_img);//各要素の平方根を求めるだけ
        cv::accumulate(tmp_img, sgm_img);//tmp_imgの要素をsgm_imgに加える
    }
    ...
    while(loop_flag){
        ...
        cv::subtract(lower_img, Zeta, lower_img);//subtract(a,b,c)はaからbを引いた結果をcに入れるだけ
        ...
        //Changed//
        //dst_imgがあらかじめ初期化されていることに注意　cv::Mat.create()を行うことで、何チャネルで読むのか、みたいなことを指定している
        //dst_img = cv::Scalar(0);
        //2.read an image from the specified file
        dst_img=cv::imread("/Users/naoto/git/opencv_gl/opencv/background_img.jpg",1);
        if(dst_img.empty()){
            fprintf(stderr,"cannot open image file\n");
            exit(0);
        }
        //end:Changed//
        frame.copyTo(dst_img, msk_img);//frameのうち、マスクした領域だけをdstへコピ-
